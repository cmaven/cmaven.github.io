...
## [12-1] 다중 IP 서버 — 측정할 링크(IP)를 명시하기

한 대의 서버에 NIC가 두 장 — **A: 인터넷·관리용**(보통 default route를 가짐), **B: 데이터 테스트용**(25G 링크) — 처럼 **여러 IP를 동시에 사용**하는 환경에서는, iperf3가 어느 인터페이스로 트래픽을 내보내는지 반드시 지정해야 한다. 아무 옵션 없이 호스트명이나 인터넷·관리용 IP(A)로 측정하면 트래픽이 **기본 경로(default route)인 A NIC**로 흘러, 25G 데이터 링크(B)가 아닌 엉뚱한 속도가 측정된다. 그래서 **B를 테스트할 때는 iperf3를 B의 IP에 바인딩(`-B`)** 해, 트래픽이 A(인터넷용)로 새지 않게 고정해야 한다.
...
| 대역 | 인터페이스 | 용도 |
|------|-----------|------|
| `10.10.10.0/24` (default 경로) | `ens259f0` | **A: 인터넷·관리(SSH)용** — 측정 대상 아님 |
| `192.168.90.0/24` | `ens801f0np0` | **B: 25G 데이터 링크 (측정 대상)** |

> SSH 접속은 `10.10.10.0/24` 대역으로 하더라도, 링크 속도는 반드시 **`192.168.90.0/24`(25G 링크)** 로 측정해야 한다.

**1) 측정 트래픽이 25G 링크로 나가는지 먼저 확인:**

```bash
# 목적지로 가는 실제 경로 확인 → dev ens801f0np0 / src 192.168.90.200 이어야 함
...
> `dev`가 `ens801f0np0`(25G)이면 정상. `ens259f0`(관리망)으로 나가면 목적지 IP나 라우팅을 다시 확인한다.

**2) 서버 B(수신): 25G IP에 바인딩해 수신 대기**

```bash
# -B로 25G 링크 IP에 바인딩 (관리 IP로 들어오는 요청은 받지 않음)
iperf3 -s -B 192.168.90.201
```

**3) 서버 A(송신): 목적지·출발지 모두 25G IP로 지정**

```bash
# -c 목적지를 B의 25G IP로, -B로 출발지를 A의 25G IP로 고정
iperf3 -c 192.168.90.201 -B 192.168.90.200 -P 8 -t 30
```

| 옵션 | 역할 |
|------|------|
| `-c 192.168.90.201` | 목적지를 25G 대역으로 지정 → 라우팅이 `ens801f0np0`로 송신 |
| `-B 192.168.90.200` | 출발지 IP를 25G 링크로 고정 (다중 IP에서 엉뚱한 NIC로 새는 것 방지) |

:warning: 호스트명이나 관리 IP(`10.10.10.x`)를 목적지로 주면 측정이 관리망 NIC로 흘러 **25G가 아닌 관리망 속도(예: 1G/10G)** 가 찍힌다. 반드시 데이터망(`192.168.90.x`) IP를 사용한다.
{: .notice--warning}

**4) 측정 중 실제 사용 인터페이스 확인:**

```bash
# 측정 중 ens801f0np0(25G)의 송수신량만 증가하는지 확인
sar -n DEV 1 | grep -E 'ens801f0np0|ens259f0'
```

`ens801f0np0`의 rx/tx만 25G 수준으로 증가하고, `ens259f0`(관리망)은 거의 변화가 없어야 정상이다.

이렇게 IP를 명시한 뒤, 아래 기본 측정 절차를 그대로 진행하면 된다(이 글의 나머지 예시는 설명을 위해 `10.0.0.x` 단일 대역을 사용한다).
