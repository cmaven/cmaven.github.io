...
tags: [Jekyll, sitemap, SEO, Google-Search-Console, sitemap-false, description, 메타태그, 404, canonical, 색인최적화, SEO-시리즈-3편, SEO-시리즈]
---

:bulb: SEO 시리즈 3편. **[1편](/github_blog/gitblog-seo-google-search/)** 에서 포스트 단위 SEO 기본기를, **[2편](/github_blog/jekyll-sitemap-google-indexing-fix/)** 에서 `_config.yml` 3줄 수정으로 sitemap이 제대로 생성·갱신되도록 만들었다. 이제 sitemap이 동작하니, 다음 단계는 **sitemap에 무엇이 들어가는지 — 즉 sitemap의 품질 자체**를 점검하는 것이다. 이 글은 저장소 전체를 SEO 감사하면서 발견한 5가지 문제와 일괄 수정 방법을 정리한다.
{: .notice--info}
...
- **3편 (이 글)** — 사이트 단위 SEO 감사: sitemap 품질, 검증 메타태그, 누락 description 일괄 보강, 404/아카이브 정리
...
2편에서 `url`·`future`·`timezone` 설정을 완료하고 sitemap이 올바른 도메인으로 생성·갱신되게 했지만, Google Search Console에 등록하기 전에 **sitemap 품질 자체**를 점검할 필요가 있었다.
...
:bulb: 1편(포스트 단위 SEO) → 2편(`_config.yml`로 sitemap 갱신 확보) → 이 글(sitemap 품질 개선) 까지 적용하면, Google이 sitemap을 신뢰하고 정기적으로 재읽기할 가능성이 크게 높아진다.
{: .notice--info}
...
[4편](/github_blog/search-console-noindex-redirect-cleanup/) 에서는 이 4가지 분류를 어떻게 해석하고, **`noindex` 메타태그 자동 출력**과 **navigation trailing slash 정리**로 깔끔하게 마무리하는 과정을 다룬다.
