...
- **2편 (이 글)** — sitemap이 제대로 갱신되도록: `_config.yml`의 url·future·timezone 3줄 진단·수정
...
`_config.yml` 3줄 수정으로 sitemap이 제대로 생성·갱신되기 시작하면, 다음 단계는 **sitemap의 내용물 자체**를 점검하는 것이다. 자동 생성된 sitemap에는 포스트뿐 아니라 **카테고리/태그/pagination 같은 저가치 archive URL**이 함께 들어가 있어서, Google이 sitemap 전체를 저품질로 판단할 수 있다.

[3편](/github_blog/jekyll-sitemap-seo-audit-fix/) 에서는 저장소 전체를 SEO 감사하면서 발견한 5가지 문제(저가치 URL 혼재, 검증 메타태그 누락, description 누락 9개, 404 페이지 부재, C/C++ 옛 글 미정리)와 일괄 수정 방법을 정리한다. 이어 [4편](/github_blog/search-console-noindex-redirect-cleanup/) 은 sitemap 등록 이후 Search Console에 남는 4가지 분류(404/리디렉션/크롤링됨-색인 안 됨/검색됨-크롤링 안 됨)의 잔여 이슈를 `noindex` 메타태그·trailing slash 정리로 마무리한다.
