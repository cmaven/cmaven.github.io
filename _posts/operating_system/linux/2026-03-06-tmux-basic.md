...
date: 2026-03-06
categories: Linux
tags: [tmux, terminal, session, pane, window, 화면분할, 터미널멀티플렉서, scroll, copy-mode]
---
...
tmux는 기본적으로 마우스 휠 스크롤이 되지 않는다. **Copy Mode**로 진입해야 위아래로 스크롤할 수 있다.

## 6-1. Copy Mode 진입/종료

```
Ctrl + b → [     # Copy Mode 진입 (스크롤 활성화)
q                # Copy Mode 종료 (스크롤 비활성화)
```

:bulb: Copy Mode 진입 후 터미널 하단에 `[0/100]` 같은 스크롤 위치 표시가 나타난다.
{: .notice--info}

## 6-2. Copy Mode 내 이동 단축키

| 단축키 | 설명 |
|---|---|
| `↑ / ↓` | 한 줄씩 위/아래 이동 |
| `Page Up / Page Down` | 한 화면씩 위/아래 이동 |
| `Ctrl + u / Ctrl + d` | 반 화면씩 위/아래 이동 |
| `g` | 버퍼 맨 위로 이동 |
| `G` | 버퍼 맨 아래로 이동 |
| `q` | Copy Mode 종료 |
...
매번 `Ctrl+b [` 없이 마우스 휠로 스크롤하려면 `~/.tmux.conf`에 아래를 추가한다:
...
:bulb: 마우스 모드를 켜면 마우스 클릭으로 패널 전환도 가능해진다.
{: .notice--info}
...
| | `Ctrl+b x` | 현재 패널 닫기 |
| | `Ctrl+b Space` | 레이아웃 순환 |
| **스크롤** | `Ctrl+b [` | Copy Mode 진입 (스크롤 활성화) |
| | `q` | Copy Mode 종료 (스크롤 비활성화) |
